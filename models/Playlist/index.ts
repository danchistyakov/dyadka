...
export const $season = root.createStore<number>(1)
  .on(setSeason, (_, season) => season)

export const $episodes = root.createStore<IEpisode[]>([])
  .on(setEpisodes, (_, season) => season.episodes)

export const $episode = root.createStore<number>(1)
  .on(setEpisode, (_, episode) => episode)

$episode.watch(data => console.log(data));
