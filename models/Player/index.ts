...
import {formatProgress, formatTime} from '@components/Players/components/DyadkaPlayer/utils/PlayerUtils';
...
setDuration.watch(data => console.log(data))

export const $isVisible = createStore<boolean>(false)
  .on(setVisibility, (_, value) => value)

export const $progress = createStore<any>({played: 0, playedFormatted: '0:00'})
  .on(setProgress, (_, data) => formatProgress(data))

export const $duration = createStore<string>('0:00')
  .on(setDuration, (_, value) => formatTime(value))

export const $isBuffering = root.createStore<boolean>(true)
  .on(setBuffering, (_, isBuffering: boolean) => isBuffering)
  .on(setTranslation, (_) => true)
  .on(setEpisode, (_) => true)
...
export const $speed = root.createStore<number>(1)
  .on(setSpeed, (_, speed: number) => speed)

export const $volume = root.createStore<number>(1)
