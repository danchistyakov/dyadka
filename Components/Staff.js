            ...
            <section className={style.staff_container}>
                {staff?.length > 2 && (
                    ...
                    >
                        ...
                        {staff?.map((res, key) => (
                            <SwiperSlide key={res?.posterUrl} className={style.staff_item} ref={node => setWidth(node?.offsetWidth)}>
                                <LazyLoadImage
                                    src={`https://cdn.statically.io/img/${res?.posterUrl.slice(8, 35)}/f=auto,w=${width},q=60/${res?.posterUrl.slice(36)}`}
                                    className={style.staff_poster}
                                    effect="blur"
                                />
                                {res?.nameRu !== undefined ? <p className={style.staff_name}>{res?.nameRu}</p> : <Skeleton count={1} duration={2} width={'9vw'} height={'2vw'} style={{ margin: '1vw' }} />}
