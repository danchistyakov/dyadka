...
const Genre = () => {
    ...
    useEffect(() => {
        const Fetch = async () => {
            ...
            const response = await fetch(`https://kinopoiskapiunofficial.tech/api/v2.1/films/search-by-filters?genre=${genre}&page=${number}`, {
                headers: {
                    "X-API-KEY": "ceff3505-c77c-450a-8abb-aa29f638f5ee"
                }
            });
            ...
            <div className={style.genre_section}>
                {result?.films?.map((res, key) => (
                    <div className={style.genre_item} key={key}>
                        <Link href={`/${res?.nameRu?.includes("сериал") ? "tv-series" : "movies"}/${res?.filmId}`}>
                            <Image className={style.genre_poster} alt={res?.nameRu} src={`${res?.posterUrl}`} />
